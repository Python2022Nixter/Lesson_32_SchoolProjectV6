...
class School:
    ...
    def __import_from_csv (self, path_to_csv: str, sql: str) -> None:
        ...
        with open(path_to_csv) as f:
            rows_from_file = list (filter(lambda x: x[0].isdigit(), f.readlines()))         
            pass
        ...
        try:
            ...
            with sqlite3.connect(self.PATH_TO_FILE_DATABASE) as conn:
                cursor = conn.cursor() # Create cursor object to operate with DB
                self.last_message += F"\n{datetime.datetime.now()}\t{c.SET_INFO_MESSAGE_COLOR}Import started{c.RESET_COLOR}\n"
            ...
            self.last_message += F"\n{datetime.datetime.now()}\t{c.SET_INFO_MESSAGE_COLOR}Import from CSV ended{c.RESET_COLOR}\n"
            pass 
    ...
    def __create_table(self, sql: str) -> None:
        self.last_message = F"\n{datetime.datetime.now()}\t{c.SET_INFO_MESSAGE_COLOR}Wait ...{c.RESET_COLOR}\n"
        try:
            with sqlite3.connect(self.PATH_TO_FILE_DATABASE) as conn:
                cursor = conn.cursor()
            ...
            self.last_message += F"\n{datetime.datetime.now()}\t{c.SET_INFO_MESSAGE_COLOR}Creating table ended{c.RESET_COLOR}\n"
            pass 
        
        pass # END __create_table(self, sql: str)
    
    def __drop_table(self, sql: str) -> None:
        self.last_message = F"\n{datetime.datetime.now()}\t{c.SET_INFO_MESSAGE_COLOR}Wait ...{c.RESET_COLOR}\n"
        self.last_message += F"\n{datetime.datetime.now()}\t{c.SET_INFO_MESSAGE_COLOR}Dropping table {c.RESET_COLOR}\n"
        try:
            with sqlite3.connect(self.PATH_TO_FILE_DATABASE) as conn:
                cursor = conn.cursor()
            ...
            self.last_message += F"\n{datetime.datetime.now()}\t{c.SET_INFO_MESSAGE_COLOR}Dropping table ended{c.RESET_COLOR}\n"
            pass 
        pass # END __drop_table(self, sql: str) -> None:
